...
fixedClients = fixedClients.concat(
  //readConfig("fixedClients", "").toString().split(', ')
...
function init() {
  ...
  registerKeys();
  var desks = readConfig("numDesks", 5);
  if (desks < 1) {
    desks = 1;
  }
  ws.desktops = desks;
  ...
  registerShortcut(
    ...
    registerShortcut(
      ...
      function() {
        ...
        var scr = client.screen;
        var x = Math.round(screenWidth(scr) * 0.0);
        var y = Math.round(screenHeight(scr) * 0.01);
        if (client.included && ignoredScreens.indexOf(scr) === -1) {
          resizeClient(client, scr, x, y);
          tileClients();
      ...
      registerShortcut(
        ...
        function() {
          ...
          var scr = client.screen;
          var x = Math.round(screenWidth(scr) * 0.01);
          var y = Math.round(screenHeight(scr) * 0.0);
          if (client.included && ignoredScreens.indexOf(scr) === -1) {
            resizeClient(client, scr, x, y);
            tileClients();
          } else {
